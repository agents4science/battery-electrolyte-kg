#!/usr/bin/env python3
"""
Integrate Electrolytomics dataset into the Knowledge Graph.

This script:
1. Loads the existing KG (v5)
2. Ingests Electrolytomics conductivity, ionization energy, and CE data
3. Saves the updated KG (v6)
"""
...
from src.ingestion.electrolytomics import ElectrolytomicsIngestor
...
def load_kg(path: Path) -> KnowledgeGraph:
    ...
    kg = KnowledgeGraph(version=data.get('version', '0.5.0'))
...
def main():
    # Paths
    KG_INPUT = PROJECT_ROOT / "data" / "output" / "knowledge_graph_v5.json"
    KG_INPUT_GZ = PROJECT_ROOT / "data" / "output" / "knowledge_graph_v5.json.gz"
    ELYTOMICS_DATA = PROJECT_ROOT / "data" / "external" / "electrolytomics"
    KG_OUTPUT = PROJECT_ROOT / "data" / "output" / "knowledge_graph_v6.json"
    ...
    print(f"  Formulations: {baseline_stats['num_formulations']}")
    print(f"  Measurements: {baseline_stats['num_measurements']}")
    print(f"  Relations: {baseline_stats['num_relations']}")

    # Run ingestion
    print(f"\nIngesting Electrolytomics data...")
    ingestor = ElectrolytomicsIngestor(kg)
    ...
    print("\nConductivity Data:")
    cond_stats = stats.get("conductivity", {})
    print(f"  Records processed: {cond_stats.get('records_processed', 0)}")
    print(f"  Records skipped: {cond_stats.get('records_skipped', 0)}")
    print(f"  Formulations created: {cond_stats.get('formulations_created', 0)}")
    print(f"  Measurements created: {cond_stats.get('measurements_created', 0)}")

    print("\nIonization Energy Data:")
    ie_stats = stats.get("ionization_energy", {})
    print(f"  Records processed: {ie_stats.get('records_processed', 0)}")
    print(f"  Records skipped: {ie_stats.get('records_skipped', 0)}")
    print(f"  Molecules created: {ie_stats.get('molecules_created', 0)}")
    print(f"  Measurements created: {ie_stats.get('measurements_created', 0)}")

    print("\nCoulombic Efficiency Data:")
    ce_stats = stats.get("coulombic_efficiency", {})
    print(f"  Records processed: {ce_stats.get('records_processed', 0)}")
    print(f"  Records skipped: {ce_stats.get('records_skipped', 0)}")
    print(f"  Formulations created: {ce_stats.get('formulations_created', 0)}")
    print(f"  Measurements created: {ce_stats.get('measurements_created', 0)}")
    ...
    print(f"  Formulations: {final_stats['num_formulations']} (+{final_stats['num_formulations'] - baseline_stats['num_formulations']})")
